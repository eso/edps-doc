...
\item \textbf{Workflow} and \textbf{reduction cascades}. A workflow is
  ...
  depending on the scientific needs.  Foe example, the same workflow
...
\item \textbf{Dataset}. A dataset is a collection of files, that are
  ...
  plus the calibrations needed to process them. In EDPS, the dataset
  has a hierarchical structure, which highlights the connections
...
\item \textbf{Target} and \textbf{Target category}. The "target", or
  ...
  this case, EDPS does not processes the science, as it has reached
  already the end reduction point (e.g., process all biases).  The
  "Target category" is a group of target that have similar
  ...
  calibrations (e.g., bias, standard stars).
