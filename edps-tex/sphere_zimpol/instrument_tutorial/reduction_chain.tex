...
%If raw frames are provided, 
A pre-processing step on the raw cubes is performed with \texttt{sph\_zpl\_preproc\_imaging} to generate pre-processed frames.
...
In practice, master bias and master dark frames are rarely used in the current imaging workflow:

\begin{itemize}
\item Bias frames are not used for the reduction of science imaging observations.
In the current imaging workflow, they are required only for the photometric
standard-star task, \textbf{zimpol\_standard\_flux\_imaging}, and are otherwise
used mainly for detector monitoring.
\item Dark frames are associated only with long exposures, defined as
\texttt{EXPTIME} $\geq 30$\,s.
\end{itemize}
...
determine the stellar position behind the coronagraph and produce calibrated star-center products.
...
%for astrometric and photometric calibration purposes.
...
\begin{itemize}
\item \textbf{zimpol\_science\_imaging}, which reduces the science imaging observations;
\item \textbf{zimpol\_science\_flux\_imaging}, which reduces off-axis flux observations when available.
\end{itemize}

The products from zimpol\_science\_flux\_imaging are used to measure the target flux in off-axis observations and are not themselves the final science products.

Optional calibration inputs include master dark, and intensity flat-field frames for each camera.
A pre-processing step on the raw cubes is performed with \texttt{sph\_zpl\_preproc\_imaging} to generate pre-processed frames.

The pre-processed science frames are then calibrated by subtracting the master dark frames
...
The center of rotation is taken from the star-center calibration frames (\texttt{ZPL\_STAR\_CENTER\_IMG\_CAM1/2}) when available;
...
The task zimpol\_science\_imaging produces the final reduced science products for both ZIMPOL cameras.
