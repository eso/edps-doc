...
by a workflow parameter). Other tasks are not generally used for scientific reduction (they are indicated by the flag "no") 
; they can be executed only by specifying them as target. Note that, when a task is specified as target, all the 
tasks that generates the calibrations needed for it are automatically executed.
