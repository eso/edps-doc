...
| zimpol_flat_imaging_cam1 | sph_zpl_intensity_flat_imaging | optional | Generates the imaging intensity flat-field calibration for camera 1. Associated using the camera-1 flat matching rule (`zpl_ff_cam1`). |
| zimpol_flat_imaging_cam2 | sph_zpl_intensity_flat_imaging | optional | Generates the imaging intensity flat-field calibration for camera 2. Associated using the camera-2 flat matching rule (`zpl_ff_cam2`). |
| zimpol_standard_astrometry_imaging | sph_zpl_science_imaging | optional | Reduces astrometric standard-field observations for calibration monitoring. Uses darks, camera-matched flats, and the filter table. |
| zimpol_standard_flux_imaging | sph_zpl_science_imaging | no | Reduces photometric standard-star observations for zeropoint monitoring. Uses darks, flats, filter table, photometric-star table, and imaging bias. |
| zimpol_coronagraph_center_imaging | sph_zpl_star_center_img | yes | Determines the stellar position behind the coronagraph for accurate centering and rotation-center definition. Uses darks and optional flats. |
| zimpol_science_flux_imaging | sph_zpl_science_imaging | optional | Reduces off-axis flux frames associated with imaging science observations; mainly used as auxiliary input for coronagraphic reductions. |
| zimpol_science_imaging | sph_zpl_science_imaging | yes | Main ZIMPOL imaging science reduction task. Uses darks, optional flats, optional coronagraph-center products, optional flux frames, and the filter table. |
...
| zimpol_dark_polarimetry | sph_zpl_master_dark | optional | Produces the polarimetric master dark for both cameras. Associated only for SlowPolarimetry exposures with exposure time > 30 s. |
...
| zimpol_flat_polarimetry_cam1 | sph_zpl_intensity_flat | optional | Generates the polarimetric intensity flat for camera 1. Uses bias only in FastPolarimetry and dark only for long SlowPolarimetry exposures. |
| zimpol_flat_polarimetry_cam2 | sph_zpl_intensity_flat | optional | Generates the polarimetric intensity flat for camera 2. Uses bias only in FastPolarimetry and dark only for long SlowPolarimetry exposures. |
...
| zimpol_modem_efficiency_cam1 | sph_zpl_modem_efficiency | optional | Generates the modulation/demodulation efficiency calibration for camera 1. Associated using specific modem-efficiency matching rules. |
| zimpol_modem_efficiency_cam2 | sph_zpl_modem_efficiency | optional | Generates the modulation/demodulation efficiency calibration for camera 2. Associated using specific modem-efficiency matching rules. |
